...
class EncodeDecode(nn.Module):
    def __init__(self, img_size):
        ...
        # graph convolution
        self.gconv1 = GCNConv(10, 128)
        self.gconv2 = GCNConv(128, 10)
    ...
    def forward(
        ...
        # compute knn graphs
        neighbor_centroids = neighbor_centroids.view(batch_size * k, -1)
        edge_idx_current = radius_graph(current_centroids, r=100)
        # compute edge weights
        current_src = current_centroids[edge_idx_current[0]]
        current_dst = current_centroids[edge_idx_current[1]]
        edge_attr_current = torch.norm(current_src - current_dst, p=2, dim=-1)
        edge_idx_neighbors = radius_graph(neighbor_centroids, r=50)
        neighbor_src = neighbor_centroids[edge_idx_neighbors[0]]
        neighbor_dst = neighbor_centroids[edge_idx_neighbors[1]]
        edge_attr_neighbors = torch.norm(neighbor_src - neighbor_dst, p=2, dim=-1)
        # graph convolution
        current_feature = lstm_out.squeeze()
        current_x = self.gconv1(current_feature, edge_idx_current, edge_attr_current)
        current_x = F.relu(current_x)
        current_x = self.gconv2(current_x, edge_idx_current, edge_attr_current)
        neighbor_feature = y.view(batch_size * k, -1)
        neighbor_x = self.gconv1(
            neighbor_feature, edge_idx_neighbors, edge_attr_neighbors
        )
        neighbor_x = F.relu(neighbor_x)
        neighbor_x = self.gconv2(neighbor_x, edge_idx_neighbors, edge_attr_neighbors)
        neighbor_x = neighbor_x.view(batch_size, k, -1)
        # graph convolved similarity matrix
        current_x_norm = F.normalize(current_x, p=2, dim=-1)
        neighbor_x_norm = F.normalize(neighbor_x, p=2, dim=-1)
        similarity_matrix_gconv = torch.bmm(
            current_x_norm.unsqueeze(1), neighbor_x_norm.transpose(1, 2)
        )
        ...
        # )
        weighted_similarity_matrix = similarity_matrix_gconv * dist_weights
        probability_matrix = F.softmax(weighted_similarity_matrix, dim=-1)
        # probability_matrix = similarity_matrix_gconv * dist_weights
...
class EncodeDecodeModel(L.LightningModule):
    ...
    def forward(
        ...
        return self.model(
            img_seq_t, img_tp1, dist_to_neighbors, current_centroids, neighbor_centroids
        )
...
def main():
    ...
    current_centroids = torch.randn(batch_size, 2)
    neighbor_centroids = torch.randn(batch_size, k, 2)
    labels = torch.randint(0, k, (batch_size, 1))
